...
- Include a semantic page structure (doctype, html, head, body, title). ok
- Add a form with:
  - A visible heading and short description. ok
  ...
  - Use `label` elements correctly linked to inputs (for accessibility). ok
  - Set the form `method` to either `GET` or `POST` and set `action` to a test endpoint (e.g., `https://httpbin.org/post` for POST or `https://httpbin.org/get` for GET) or `#` for local testing. Include a one-line comment explaining your choice. ok
  - Use HTML5 validation attributes (`required`, `pattern`, `minlength`, etc.) on at least two fields. ok
- Embed a video:
  - Use the HTML5 `<video>` element with `controls`. ok
  - Download a sample video file from https://www.pexels.com/search/videos/. ok
  - Reference a local file (e.g., `media/sample.mp4`), and include fallback text. ok
  - Optionally include a captions/subtitles `<track>` element and a short caption file (or explain how to add one). NO
